      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-center">
          ...
          <div className="lg:col-span-7 space-y-6 text-center lg:text-left">
            ...
            <div className="pt-2">
              <div className="p-2.5 sm:p-3 rounded-2xl bg-white border border-slate-200 shadow-xs flex items-center justify-between gap-2 sm:gap-3 text-xs overflow-hidden">
                <div className="flex items-center gap-2 min-w-0 flex-1">
                  <span className="px-2 py-0.5 rounded-md bg-emerald-100 text-emerald-800 font-bold text-[10px] shrink-0 whitespace-nowrap">
                    LIVE UPDATE
                  </span>
                  <p
                    className="text-slate-700 font-medium text-left text-[11px] sm:text-xs min-w-0 flex-1 truncate"
                    title={
                      recentResolved
                        ? `Resolved: ${recentResolved.title} in ${recentResolved.area}`
                        : 'Citizen spot-fix active in Indiranagar'
                    }
                  >
                    {recentResolved ? (
                      <span className="block truncate max-w-full text-slate-700 font-medium">
                        <strong className="font-semibold text-emerald-700">Resolved:</strong>{' '}
                        {recentResolved.title} in {recentResolved.area}
                      </span>
                    ) : (
                      <span className="block truncate max-w-full text-slate-700 font-medium">
                        Citizen spot-fix active in Indiranagar
                ...
                <button
                  ...
                  className="text-cyan-700 hover:underline flex items-center gap-1 text-[11px] font-bold shrink-0 whitespace-nowrap pl-1"
                >
                  <span className="hidden xs:inline">View </span>Feed <ArrowRight className="w-3 h-3" />
                </button>
              </div>
            </div>
          </div>
